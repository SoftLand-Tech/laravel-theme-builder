...
/**
 * Searches the store's published blog posts by title. The stored value is an
 * id, but the UI only ever shows the post title, never a raw id. Mirrors
 * ProductPickerField, minus the price (posts have none).
 */
export function BlogPostPickerField({ multi = false, value, onChange, endpoint = apiEndpoint('/api/blog-posts') }: BlogPostPickerProps) {
    ...
    // Resolve stored ids to titles once on mount so the user sees titles.
    useEffect(() => {
        ...
        const known = new Map<number, BlogPost>();
        Promise.all(
            ids.map((id) =>
            ...
            setSelected(ids.map((id) => known.get(id)).filter((p): p is BlogPost => Boolean(p)));
        });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const toggle = (p: BlogPost) => {
        if (multi) {
            const ids = Array.isArray(value) ? [...value] : value !== null ? [value] : [];
        ...
        <div className="space-y-2">
            ...
            {!loading && (
                <div className="max-h-48 space-y-1 overflow-y-auto">
                    {posts.map((p) => {
                        const isSelected = Array.isArray(value) ? value.includes(p.id) : value === p.id;
                        return (
                            <button
                                key={p.id}
                                type="button"
                                ...
                                className={`block w-full rounded px-2.5 py-1.5 text-left text-sm transition ${
                                    isSelected ? 'bg-primary-600 text-white' : 'text-neutral-800 hover:bg-neutral-100'
                                }`}
                            >
                                {p.name}
                            </button>
                    ...
                    {posts.length === 0 && <p className="px-2 py-2 text-xs text-neutral-500">No posts found.</p>}
                </div>
            )}
        </div>
    );
