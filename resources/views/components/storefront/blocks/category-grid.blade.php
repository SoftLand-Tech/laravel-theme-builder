...
<div class="grid grid-cols-2 gap-4 {{ $colClass }}">
    @foreach ($categories ?? [] as $category)
        ...
        <a
            href="{{ tb_route('storefront.category', ['slug' => $c['slug'] ?? '']) }}"
        ...
        >
            ...
            <div class="mt-3 text-center">
                ...
                @if (! empty($c['productCount']))
                    <span class="block text-caption text-stone">{{ $c['productCount'] }} {{ __('storefront.products') }}</span>
                @endif
            </div>
        </a>
    @endforeach
